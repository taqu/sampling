#ifndef INC_LRAY_SORT_H_
#define INC_LRAY_SORT_H_
/**
@file Sort.h
@author t-sakai
...
#define MERGE_GETMEM(NEED) ((NEED)<=allocated_? 0 : merge_getmem(NEED))
    ...
    {
        LRAY_ASSERT(lo.keys_<=start && start<=hi);
        // [lo, start)はソート済み
        if(lo.keys_ == start){
            ++start;
        }
    ...
    s32 Timsort<T, Comp>::gallop_left(const_reference_type key, pointer_type a, s32 n, s32 hint)
    {
        ...
        a -= hint;
        LRAY_ASSERT(-1<=lastoffset && lastoffset<offset);
        LRAY_ASSERT(offset<=n);
    ...
    s32 Timsort<T, Comp>::merge_getmem(s32 need)
    {
        ...
        a_.keys_ = (pointer_type)LRAY_MALLOC(need * sizeof(value_type));
        if(NULL == a_.keys_){
            return -1;
        }
        allocated_ = need;
    ...
    {
        LRAY_ASSERT(0<na && 0<nb);
        LRAY_ASSERT(ssa.keys_ + na == ssb.keys_);
        MERGE_GETMEM(na);
...
CopyA:
    ...
    {
        LRAY_ASSERT(2<=numPendings_);
        LRAY_ASSERT(0<=n);
        LRAY_ASSERT(n == numPendings_-2 || n == numPendings_-3);
        LRAY_ASSERT(0<pending_[n].length_ && 0<pending_[n+1].length_);
    ...
    void Timsort<T, Comp>::sort(pointer_type keys, s32 size)
    {
        ...
        do{
            ...
            LRAY_ASSERT(numPendings_<MAX_MERGE_PENDING);
